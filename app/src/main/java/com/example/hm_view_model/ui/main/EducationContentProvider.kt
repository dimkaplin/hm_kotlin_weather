...
import com.example.hm_view_model.model.app.App
...
import com.example.hm_view_model.model.database.*
...
class EducationContentProvider: ContentProvider() {
    ...
    override fun query(
        ...
        val historyDao: HitoryDAO = getHistoryDao()
    ...
    private fun map(values: ContentValues?): HistoryEntity {
        return if (values == null) {
            HistoryEntity(0,"",null, null)
        } else {
            val id = if (values.containsKey(ID)) values[ID] as Long else 0
            val city = values[CITY.toString()] as String
            val temperature = values[TEMPERATURE] as Int
            HistoryEntity(id, city, temperature, null)
        }
    }
